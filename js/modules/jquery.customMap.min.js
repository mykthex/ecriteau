...
(function($) {
    ...
    $.extend(CustomMap.prototype, {
        ...
        loadAPI: function() {
            ...
            var key = 'AIzaSyBAB_IQygykRZVlSgBTxyiJ8rKn4KDB-Yw';
            $.getScript('https://www.google.com/jsapi', $.proxy(function() {
                if (!window.google.maps) {
                    google.load('maps', '3', {
                        other_params: 'sensor=false&key=' + key,
        ...
        addMarker: function(marker) {
            ...
            if (marker.infoWindowContent != '' && marker.infoWindowContent != null) {
                ...
                this.bindCustomMarkerEvents(markerObj);
            }

            // If first marker active
            if (this.config.markerActive !== null) {
                if (marker.id == this.config.markerActive) {
                    markerObj.isClicked = true;
                    this.changeMarkerIcon(markerObj, 'hover');
                    $('[data-id="' + marker.id + '"]').addClass(this.config.classes.states.active);
                }
            }
        },

        //Added by me
        bindCustomMarkerEvents: function(marker) {
            marker.addListener('click', $.proxy(function() {
                ...
                // Activate marker
                if ($('[data-id="' + marker.customId + '"]').hasClass(this.config.classes.states.active)) {
